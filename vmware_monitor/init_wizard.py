"""Interactive first-run setup wizard — ``vmware-monitor init``.
...
Read-only with respect to vSphere — it only touches local config files.
"""
...
from vmware_monitor.config import (
    CONFIG_DIR,
    CONFIG_FILE,
    ENV_FILE,
...
def run_init(force: bool = False, skip_test: bool = False) -> int:
    ...
    console.print("[bold cyan]vmware-monitor init[/] — guided setup\n")
    ...
    if skip_test:
        console.print("\nNext: [cyan]vmware-monitor doctor[/] to verify the connection.")
        return 0

    if not typer.confirm("\nTest the connection now?", default=True):
        console.print("Next: [cyan]vmware-monitor doctor[/] to verify the connection.")
        return 0
